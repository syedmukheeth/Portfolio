      ...
      <div className="container max-w-6xl mx-auto px-4 pt-40 pb-20">
        ...
        <section className="mb-60">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {[1, 2].map((num) => (
                <VideoCard key={num} src={`/clips/PeerNet-${num}.mp4`} num={num} />
              ))}
           </div>
        </section>
    ...
    >
       ...
       <div className={cn(
         ...
         isHovered ? "opacity-0" : "opacity-100"
          ...
          <div className="flex flex-col items-center gap-4">
             <div className="w-12 h-12 rounded-full border border-blue-400/20 flex items-center justify-center text-blue-400/40 animate-pulse">
                <Radio size={20} />
             </div>
       ...
       <div className="absolute bottom-6 left-6 flex items-center gap-3 pointer-events-none">
          ...
          <div className="text-[10px] font-mono text-white/40 uppercase tracking-widest">
             {isHovered ? "Live_Interaction_Trace" : "Static_Analysis"}
          </div>
       </div>
       ...
       <div className="absolute top-6 right-6 flex flex-col items-end gap-1 pointer-events-none">
          <div className="w-12 h-1 bg-blue-500/40 rounded-full overflow-hidden">
             <motion.div 
               animate={{ width: isHovered ? ["20%", "80%", "40%"] : "10%" }} 
               transition={{ duration: 3, repeat: Infinity }} 
               className="h-full bg-blue-400" 
             />
          </div>
          <div className="text-[8px] font-mono text-blue-400/60 uppercase">
             {isHovered ? "Buffering_Stream" : "Idle_State"}
          </div>
       </div>
    </div>
  );
